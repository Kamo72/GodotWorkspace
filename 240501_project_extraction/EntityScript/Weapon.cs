...
public partial class Weapon : Node2D
{
	...
	public override void _Ready()
	{
		...
		magNow = weaponStatus.detailDt.basicMag;
		magMax = weaponStatus.detailDt.basicMag;
    }
	...
	void Fire()
    {
		cooldown += (float)60 / weaponStatus.detailDt.roundPerMinute;
		magNow--;

        int count = weaponStatus.detailDt.pellitAmount > 2? weaponStatus.detailDt.pellitAmount : 1;
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            float moaValue = 0f;//(random.NextSingle() - 0.5f) * 2f * weaponStatus.aimDt.ads.moa * (float)Math.PI / 180f;
            proj.GlobalRotation = GlobalRotation + moaValue;
            
            proj.speed = weaponStatus.detailDt.muzzleSpeed;
            proj.damage = weaponStatus.detailDt.basicDamage;
